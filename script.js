  ...
  reader.onload = () => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // Try free space by removing other saved photos
        const keys = [];
        for (let i = 0; i < localStorage.length; i++) {
          const k = localStorage.key(i);
          if (k && k.startsWith('itskamyar-') && k !== 'itskamyar-' + currentEditKey && k !== 'itskamyar-comments' && k !== 'itskamyar-visits' && k !== 'itskamyar-last-active') {
            keys.push(k);
          }
        }
        // Remove half of them
        ...
        try {
          ...
          console.warn('localStorage full', err2);
          alert('Storage still full. Click "Clear Saved Photos" then try again.');
        }
      }
    };
...
const OPERATORS = [
  // ATTACKERS - S
  { name: 'Ace', img: 'img/ace.jpg', side: 'atk', tier: 'S', org: 'NIGHTHAVEN', ability: 'S.E.L.M.A. Aqua Breacher — hard breach device that creates line of sight holes.' },
  { name: 'Dokkaebi', img: 'img/dokkaebi.jpg', side: 'atk', tier: 'S', org: '707th SMB', ability: 'Logic Bomb — calls enemy phones, reveals positions and distracts.' },
  { name: 'Grim', side: 'atk', tier: 'S', org: 'NIGHTHAVEN', ability: 'Kawan Hive Launcher — deploys tracking insects that reveal enemies.' },
  { name: 'Montagne', img: 'img/montagne.jpg', side: 'atk', tier: 'S', org: 'GIGN', ability: 'Extendable Shield — full-body coverage for entry and plant protection.' },
  { name: 'Nomad', img: 'img/nomad.jpg', side: 'atk', tier: 'S', org: 'GIGR', ability: 'Airjab Launcher — repulsion grenades that knock back enemies.' },
  { name: 'Ram', img: 'img/ram.jpg', side: 'atk', tier: 'S', org: 'NIGHTHAVEN', ability: 'BU-GI Auto-Breacher — robotic device that destroys floors above.' },
  { name: 'Solid Snake', img: 'img/solidsnake.jpg', side: 'atk', tier: 'S', org: 'FOXHOUND', ability: 'Cardboard Box / stealth tools — infiltration and intel (crossover).' },
  { name: 'Striker', img: 'img/striker.jpg', side: 'atk', tier: 'S', org: 'ROS', ability: 'Flexible loadout — customizable primary gadgets for any role.' },
  // ATTACKERS - A
  { name: 'Ash', img: 'img/ash.jpg', side: 'atk', tier: 'A', org: 'FBI SWAT', ability: 'M120 CREM — breaching rounds that destroy soft walls and gadgets.' },
  { name: 'Blackbeard', img: 'img/blackbeard.jpg', side: 'atk', tier: 'A', org: 'Navy SEALs', ability: 'Rifle-Shield — ballistic shield on primary weapon.' },
  { name: 'Deimos', img: 'img/deimos.jpg', side: 'atk', tier: 'A', org: 'Unknown', ability: 'Deathmark Tracker — marks and hunts a chosen target.' },
  { name: 'Lion', img: 'img/lion.jpg', side: 'atk', tier: 'A', org: 'GIGN', ability: 'EE-ONE-D — drone scan that reveals moving enemies.' },
  { name: 'Maverick', img: 'img/maverick.jpg', side: 'atk', tier: 'A', org: 'GSUTR', ability: 'Breaching Torch — silently creates peep holes in reinforced walls.' },
  { name: 'Thatcher', img: 'img/thatcher.jpg', side: 'atk', tier: 'A', org: 'SAS', ability: 'EMP Grenade — disables electronic gadgets in radius.' },
  { name: 'Twitch', img: 'img/twitch.jpg', side: 'atk', tier: 'A', org: 'GIGN', ability: 'Shock Drone — destroys gadgets and gathers intel remotely.' },
  { name: 'Ying', img: 'img/ying.jpg', side: 'atk', tier: 'A', org: 'SDU', ability: 'Candela — cluster flash charges that blind defenders.' },
  { name: 'Zofia', img: 'img/zofia.jpg', side: 'atk', tier: 'A', org: 'GROM', ability: 'KS79 LIFELINE — impact grenades and concussions.' },
  { name: 'Blitz', img: 'img/blitz.jpg', side: 'atk', tier: 'A', org: 'GSG 9', ability: 'Flash Shield — blinding flashes while advancing with shield.' },
  // ATTACKERS - B
  ...
  { name: 'Capitao', img: 'img/capitao.jpg', side: 'atk', tier: 'B', org: 'BOPE', ability: 'Tactical Crossbow — fire and smoke bolts for area control.' },
  { name: 'Flores', img: 'img/flores.jpg', side: 'atk', tier: 'B', org: 'Unauthorized', ability: 'RCE-Ratero Charge — remote explosive drone.' },
  { name: 'Finka', side: 'atk', tier: 'B', org: 'Spetsnaz', ability: 'Adrenal Surge — team-wide health boost and revive assist.' },
  { name: 'Gridlock', img: 'img/gridlock.jpg', side: 'atk', tier: 'B', org: 'SASR', ability: 'Trax Stingers — area denial spikes that slow enemies.' },
  { name: 'Hibana', img: 'img/hibana.jpg', side: 'atk', tier: 'B', org: 'SAT', ability: 'X-KAIROS — pellet charges that open reinforced walls.' },
  { name: 'IQ', img: 'img/iq.jpg', side: 'atk', tier: 'B', org: 'GSG 9', ability: 'Electronics Detector — sees gadgets through walls.' },
  { name: 'Thermite', img: 'img/thermite.jpg', side: 'atk', tier: 'B', org: 'FBI SWAT', ability: 'Exothermic Charge — large hard-breach openings.' },
  // ATTACKERS - C
  { name: 'Osa', img: 'img/osa.jpg', side: 'atk', tier: 'C', org: 'NIGHTHAVEN', ability: 'TALON-8 Clear Shield — deployable transparent hard shield.' },
  // ATTACKERS - D
  ...
  { name: 'Smoke', img: 'img/smoke.jpg', side: 'def', tier: 'A', org: 'SAS', ability: 'Remote Gas Grenade — toxic area denial.' },
  ...
  { name: 'Mute', img: 'img/mute.jpg', side: 'def', tier: 'B', org: 'SAS', ability: 'Signal Disruptor — jams drones and electronic gadgets in a radius.' },
      ...
      <div class="card-3d" data-side="${op.side}" data-tier="${op.tier}">
        <div class="card-inner">
          <div class="card-front">
            <div class="card-img" style="${op.img ? `background-image:linear-gradient(to bottom,rgba(0,0,0,.15),rgba(0,0,0,.7)),url('${op.img}');background-size:cover;background-position:center top;` : `background:linear-gradient(160deg,#111,#1a1a24 40%,#0d0d12);display:flex;align-items:center;justify-content:center;`}">
              ${op.img ? '' : `<span style="font-family:Orbitron,sans-serif;font-size:1.6rem;color:${tierColor(op.tier)};text-shadow:0 0 20px ${tierColor(op.tier)}">${op.tier}</span>`}
            </div>
...
// Clear all custom saved photos
document.getElementById('clear-photos-btn')?.addEventListener('click', () => {
  if (!confirm('Delete all custom photos saved in this browser? (Default images stay)')) return;
  const toRemove = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && k.startsWith('itskamyar-') && k !== 'itskamyar-comments' && k !== 'itskamyar-visits' && k !== 'itskamyar-last-active' && k !== 'itskamyar-edit-unlocked') {
      toRemove.push(k);
    }
  }
  toRemove.forEach(k => localStorage.removeItem(k));
  alert('Cleared ' + toRemove.length + ' saved photos. Refresh the page.');
  location.reload();
});
